...
FEATURES = ["Brand", "Displacement_cc", "Horsepower_PS", "Seats"]         # Example: ["Manufacture_Year", "Mileage_km"]
RANDOM_STATE = 42   # public demo — does not need to match your Student ID
@st.cache_data
def load_and_train():
    df = pd.read_csv("Lab04_hk_car_price.csv")
    # Remove any rows with empty values in features or target price
    df = df.dropna(subset=FEATURES + ["Price_HKD"])
...
st.title("HK Used Car Price Estimator — HOHOLAM_250038669_CA2 Prototype")                  #<- Change your name here!!!=======================================
